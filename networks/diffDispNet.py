...
class HRDepthDecoder(nn.Module):
    ...
    def forward(self, input_features):
        ...
        outputs[("disp",0)] = self.alpha * self.sigmoid(self.convs["dispConvScale0"](x6)) + self.beta
        outputs[("disp",1)] = self.alpha * self.sigmoid(self.convs["dispConvScale1"](x9)) + self.beta
        outputs[("disp",2)] = self.alpha * self.sigmoid(self.convs["dispConvScale2"](x18)) + self.beta
        outputs[("disp",3)] = self.alpha * self.sigmoid(self.convs["dispConvScale3"](x36)) + self.beta
        return outputs
...
class DispNet(nn.Module):
    def __init__(self):
        ...
        self.decoder = HRDepthDecoder(self.encoder.num_ch_enc, range(4))
...
if __name__ == '__main__':
    in_image = torch.rand((4, 3, 256, 320))
    depth_net = DispNet()
    disp = depth_net(in_image)
    print(disp.size())
